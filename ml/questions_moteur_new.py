...
def generer_questions(constantes: dict, symptom_text: str, age: int, sex: int) -> list:
    '''
    Geénère des questions dynamiques via Groq API (modele llama-3.3-70b-versatile).
    Renvoie une liste de questions structurees.
    '''
    # Essaie de trouver la clé d'API dans l'environnement, sinon affiche un mock pour le developpement
    api_key = os.environ.get("GROQ_API_KEY", "")
    
    if not api_key:
        print("[AVERTISSEMENT] Pas de GROQ_API_KEY définie.")
        # Questions de secours basiques piochées au hasard pour le frontend si l'API est absente
        return [
            {
                "id": "q1",
                "texte": "Avez-vous des antécédents médicaux notables ?",
                "type": "oui_non",
                "feature_name": "q_medicaments_generaux"
            },
            {
                "id": "q2",
                "texte": "Depuis combien de temps le symptôme a-t-il apparu ?",
                "type": "choix",
    ...
    # Liste des features existantes dans le modèle:
    ...
    prompt_user += "Génère exactement 4 questions médicales d'urgence pertinentes et directes à  poser à  ce patient "
    prompt_user += "pour analyser l'urgence ou préciser son symptôme principal.\n"
    prompt_user += "Donne UNIQUEMENT un tableau JSON natif contenant des objets sous la forme:\n"
    prompt_user += "[\n  {\n"
    prompt_user += '    "id": "q_id_unique",\n'
    prompt_user += '    "texte": "La question naturelle posée",\n'
    ...
    prompt_user += "  }\n]\n"
    prompt_user += "Ne met RIEN DEVANT ou APRES le JSON. Même pas de '`json' ou de texte."
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Nettoie les eventuels blocs markdown si l'IA en a généré
            if content.startswith("`"):
                content = content.replace("`json", "").replace("`", "").strip()

            questions = json.loads(content)
            # Scurité : on s'assure que c'est une liste
    ...
    # Fallback si à§a a échoué
    return [
        {
            "id": "q_secours",
            "texte": "Décrivez plus en détail vos symptômes:",
            "type": "texte_libre",
            "feature_name": "q_premiere_fois"
        }
    ]
